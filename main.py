...
async def get_staff(
    ...
    is_sort_order_asc: bool | None = None,
):
    staff = None
    if sort_column == None:
    ...
    return templates.TemplateResponse(
        "table.html", {"request": request, "data": changeDateForShow(staff)}
    )
...
async def update_employee(
    staff_id: int, employee: schema.Staff, db: Session = Depends(get_db_session)
):
...
async def search_employee(
    ...
    address = f"%{address}%"
    employeeOrStaff = (
        db.query(models.Staff_DB)
        .filter(
            models.Staff_DB.first_name.ilike(first_name),
    ...
    response = templates.TemplateResponse(
        "tableSearch.html",
        {"request": request, "searched_staff": changeDateForShow(employeeOrStaff)},
    )
    return response
